lapmm <- function(y, t, numUnit, obsPerUnit, numFixEff,
  ...
  hessianMode <- get_hessian(phi.model, inv.cov.mat = invCovMatMode)
...
model1 <- lapmm(y=y, t=t, numUnit=numUnit, obsPerUnit=obsPerUnit, numFixEff=numFixEff,
                  gammaA = 0.001, gammaB = 0.001, fixEffVar = 10^4)
model1$fix_effect
...
model2 <- lapmm(y=seizure.rate, t = t, numUnit=numUnit, obsPerUnit = obsPerUnit, 
             numFixEff = numFixEff,
             gammaA = 0.001, gammaB = 0.001, fixEffVar = 10^4)
model2$fix_effect
