labpmm2_grid <- function(y, t, r, numUnit, obsPerUnit, 
  ...
  for (v1 in v1_grid){
    for (v2 in v2_grid){
      ...
      for (j in 1:numFixEff){
        phi_grid[i, j] = phi.vector[j]
        var_grid[i, j] <- la.var[j,j]
      }
    }
  }
  ...
  var_phi <- colSums(var_grid*weights)
  ...
  results <- list(fix_effect = mean_phi, 
                  sd_fixEffect = sqrt(var_phi),
